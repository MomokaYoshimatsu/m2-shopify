...
- Metafield / Metaobjectの読み取り・定義・値変更は、Shopify GraphQL Admin APIを第一選択とする。
- 管理画面のブラウザ操作は、公式ドキュメントとGraphQL schemaでAPI非対応を確認し、理由と操作内容を説明してユーザー承認を得た場合のみ行う。
- Admin APIまたは承認済みのブラウザ操作で定義・値を変更する前に、変更内容、対象、影響を説明し、ユーザー承認を得る。
- Admin APIを使う場合も `shopify-safety.md` の管理画面変更と同じ承認条件を適用する。
- API操作前に読み取りqueryで対象ストア、既存定義、対象ID、現在値を確認し、mutation後は `userErrors` と再読み取りで反映を検証する。
- Client Secret、アクセストークン、Shopifyセッション情報をGit管理ファイル、作業ログ、commit、チャット出力へ保存しない。
- 認証情報は安全なローカル環境変数またはシークレットマネージャーから読み込み、値をコマンド引数や出力へ含めない。
